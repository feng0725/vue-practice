            ...
            <div class="clearfix">
                <div class="left fl">
                    <div style="margin-top:20px;margin-bottom:30px;">
                        <Button v-show="btnIndex==1"  class="btn-user " @click="changeType(1)" :class="{active:btnIndex==1}">用户通知
                        </Button>
                        <Button v-show="btnIndex==3" class="btn-user" @click="changeType(3)" :class="{active:btnIndex==3}">课程通知</Button>
                    </div>
                    <div v-show="btnIndex==1">
                        <Form class="from" ref="userNotification" :model="userNotification"
                            ...
                            <FormItem v-if="userNotification.userType == 2" label="选择分组(必选)" prop="groupId">
                                <Button :disabled="true" class="white-blue" @click="isGroup = true" type="primary">选择分组</Button>
                                {{groupStr}}
                            </FormItem>
                        </Form>
                    </div>
                        ...
                        <Form class="from" ref="courseNotice" :model="courseNotice" :rules="courseNoticeRules"
                            ...
                            <FormItem label="购买类型(必选)" prop="isBuy">
                                <RadioGroup  @on-change="changeIsBuy" v-model="courseNotice.isBuy">
                                    <Radio :disabled="true" v-if="!$store.getters.isEnterprise" label="0">未购买课程用户</Radio>
                            ...
                            <template v-if="courseNotice.isBuy ==0">
                                <FormItem label="用户类型(必选)" prop="userType">
                                    <Select :disabled="true" @on-change="changeUserType" v-model="courseNotice.userType"
                                            style="width:200px" placeholder="企业用户">
                                        <Option v-for="item in userType" :value="item.value" :key="item.value">{{
                                            item.label}}
                                        </Option>
...
export default {
    ...
    mounted() {
        if (this.insertNotice.enterpriseId) {
            this.enterpriseId = this.insertNotice.enterpriseId.split('#')[1];
        }
        this.init();
    },
    methods: {
