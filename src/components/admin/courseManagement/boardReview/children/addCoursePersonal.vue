<template>
    <div class="addCourseAdmin edit-new">
                ...
                <div class="left fl">
                    <Form class="from" ref="courseMsg" :model="courseMsg" :rules="ruleValidate" :label-width="180"
                        ...
                        <FormItem label="选择公众号(必选)" prop="appSelectList">
                            <span>{{this.courseMsg.appSelectList[0].name}} (默认)</span>
                        </FormItem>
        ...
        <MyDialog width="380" class-name="select-list" @ok="setAppList" :title="'选择公众号'" :visible.sync="isSelectAppList">
...
export default {
    ...
    data() {
        return {
            userAccount: this.$store.state.userInfo.userAccount,
    ...
    methods: {
        ...
        async edit() {
            if (this.courseId) {
                ...
                this.userAccount = this.courseMsg.userAccount;
                this.getEnterpriseUserList();
                this.getGroupList();
                this.getAppList();
                // 设置公众号
                this.courseMsg.appSelectList = res.obj.appidList.map((item) => {
                    let json = this.appSelectList.filter((app) => {
                        return app.appid == item.appid;
                    })[0];
                    item.name = json ? json.name : '';
                    return item;
                });
        ...
        getEnterpriseUserList() {
            this.courseMsg.lecturerId = this.operatorId;
        },
        async getAppList() {
            let res = await this.$fetch({
            ...
            if (res.code == 200) {
                this.courseMsg.appSelectList = res.obj;
                this.appSelectList = res.obj;
            }
        },
        /**
         * 获取组列表
        ...
        validateCategory() {
            if (this.categoryList2.length > 0 && !this.category2) {
                ...
                return !(this.categoryList3.length > 0 && !this.category3);
            }
        },
        next: function() {
