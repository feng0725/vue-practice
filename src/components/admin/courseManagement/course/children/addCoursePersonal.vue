    ...
    <div class="addCourseAdmin edit-new">
        ...
        <div class="wrapper clearfix">
                ...
                <div class="left fl">
                    <Form class="from" ref="courseMsg" :model="courseMsg" :rules="ruleValidate" :label-width="180"
                        ...
                        <FormItem label="选择公众号(必选)" prop="appSelectList">
                            <span>{{this.courseMsg.appSelectList[0] ? this.courseMsg.appSelectList[0].name : ''}} (默认)</span>
                        </FormItem>
                        <FormItem label="是否收费(必选)" prop="isFreeOut">
                            ...
                            <span
                                v-show="$store.state.surplus < 600 && courseMsg.isFreeOut==1 && !$store.getters.isAdmins"
                                class="warning">*您的课时余量已不够开设免费课程，请充值后再创建。</span>
                        </FormItem>
            ...
            <div class="btn-box fl">
                <Button :disabled="$store.state.surplus < 600 && courseMsg.isFreeOut==1 && !$store.getters.isAdmins"
                        class="btn fr" type="primary" @click="next">下一步
                </Button>
...
export default {
    ...
    methods: {
        ...
        async edit() {
            ...
            this.userAccount = this.courseMsg.userAccount || this.$store.state.userInfo.userAccount;
            this.getEnterpriseUserList();
            this.getGroupList();
            this.getAppList();
            // 设置公众号
