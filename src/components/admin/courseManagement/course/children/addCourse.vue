    ...
    <div class="addCourse edit-new">
        ...
        <div class="wrapper clearfix">
                ...
                <div class="left fl">
                    <Form class="from" ref="courseMsg" :model="courseMsg" :rules="ruleValidate" :label-width="180"
                        ...
                        <FormItem label=" 上课账号(必填)" prop="lecturerId">
                            <Select  style="width: 400px;" v-model="courseMsg.lecturerId" filterable>
                                <Option v-for="item in enterpriseUserList" :value="item.userId" :key="item.userId" placeholder="输入用户名查找,模糊查询">{{ item.userAccount }}</Option>
                            </Select>
                        </FormItem>
                        <FormItem label="课程范围(必选)" prop="courseType">
                            <CheckboxGroup v-model="courseSharing">
                                <Checkbox label="0">
                                    <span>内部课程</span>
                                </Checkbox>
                                <Checkbox label="1">
                        ...
                        <div class="item" v-if="isIn">
                            <h5>内部课程 <span v-show="$store.state.surplus < 600 && !$store.getters.isAdmins" class="warning">*您的课时余量已不够开设免费课程，请充值后再创建。</span></h5>

                            <FormItem label="针对分组开放" prop="mail">
                                <Button type="primary" @click="isSelectGroup = true" class="white-blue">选择分组</Button>
                                <span>{{showGroupList.join(',')}}</span>
                                <span class="warning">*选择某些分组就是本课程只对这些组的用户开放.</span>
                            </FormItem>
                            <FormItem style="margin:0" label="是否分享直播页(必选)" prop="isShareIn">
                                <RadioGroup v-model="courseMsg.isShareIn">
                                    <Radio label="1">分享</Radio>
                                    <Radio label="0">不分享</Radio>
                                </RadioGroup>
                            </FormItem>
                        </div>
                        <div class="item" v-if="isOut">
                            <h5>
                                公开课程
                                <span v-show="$store.state.surplus < 600 && courseMsg.isFreeOut==1 && !$store.getters.isAdmins"  class="warning">*您的课时余量已不够开设免费课程，请充值后台创建。</span>
                            </h5>
                            <FormItem label="选择公众号(必选)" prop="appSelectList">
                                <CheckboxGroup v-model="courseMsg.appSelectList">
                                    <Checkbox v-for="(item,index) in appList" :key="index" :label="item.appid">{{item.name}}</Checkbox>
                                </CheckboxGroup>
                            ...
                            <FormItem  style="margin:0"   v-if="courseMsg.isFreeOut =='1'" label="是否分享直播页(必选)" prop="isShareOut">
                        ...
                        <FormItem label="分类(必填)" prop="categoryId">
                            <Select  :disabled="categoryList1.length ==0" @on-change="changeCategory1" v-model="category1" style="width:200px">
                                <Option v-for="item in categoryList1" :value="item.categoryId" :key="item.categoryId">{{ item.categoryName                                 }}
                                </Option>
                            </Select>

                            <Select :disabled="categoryList2.length ==0" @on-change="changeCategory2" v-model="category2" style="width:200px">
                                <Option v-for="item in categoryList2" :value="item.categoryId" :key="item.categoryId">{{ item.categoryName                                    }}
                                </Option>
                            </Select>
                            <Select :disabled="categoryList3.length ==0" @on-change="changeCategory3" v-model="category3" style="width:200px">
                                <Option v-for="item in categoryList3" :value="item.categoryId" :key="item.categoryId">{{ item.categoryName                                    }}
                                </Option>
                            </Select>
                        ...
                        <FormItem label="课程时长(必填)" prop="courseDuration">
                            <TimePicker type="time"  v-model="courseMsg.courseDuration" placeholder="请选择课程时长" style="width: 200px"></TimePicker>

                        </FormItem>
                        <FormItem label="是否互动(必填)" prop="isInteract">
                            <Select v-model="courseMsg.isInteract" style="width:200px">
                                <Option v-for="item in isInteractList" :value="item.value" :key="item.value">{{ item.label
                                    }}
                                </Option>
                            </Select>
                        </FormItem>
            ...
            <div class="btn-box fl">
                <Button  :disabled="$store.state.surplus < 600 && courseMsg.isFreeOut==1 && !$store.getters.isAdmins" class="btn fr" type="primary" @click="next">下一步</Button>
                <Button v-if="courseMsg.draftStatus" class="btn fr white-blue" @click="saveDraft" style="margin-right: 20px" type="primary">保存草稿</Button>
            </div>
        </div>
        <MyDialog width="380" class-name="select-list" @ok="setGrouList" :title="'选择分组'" :visible.sync="isSelectGroup">
            <div style="text-align: left">
                <h4>分组</h4>
                <div class="check-box">
                    <CheckboxGroup v-model="disabledGroup">
                        <Checkbox v-for="item in groupList" :key="item.groupId" :label="item.groupId">{{item.name}}</Checkbox>
                    </CheckboxGroup>
                </div>
            </div>
        </MyDialog>
...
export default {
    ...
    data() {
        return {
            ...
            courseMsg: {
                ...
                courseName: '', // 课程名字
                lecturerId: '', // 教师ID
                courseType: '', // 课程范围 （0-内部课 1-公开课 2-内公开课）
                categoryId: '', // 分类信息
    ...
    watch: {
        ...
        courseSharing(val) {
            ...
            if (val.length == 1) {
                ...
                this.courseMsg.courseType = '';
            }
            this.$refs.courseMsg.validateField('courseType');
        }
    },
    mounted() {
        this.init();
    ...
    methods: {
        init() {
            ...
            this.edit();
            console.log(!storage.get('courseMsg'));
        ...
        async edit() {
            ...
            } else if (this.courseMsg.courseType != '') {
            ...
            this.getAppList();
            // 设置分组
            this.disabledGroup = res.obj.groupList.map((item) => item);
            this.getGroupList(this.setGrouList);
        ...
        setGrouList() {
            this.showGroupList = this.disabledGroup.map((item) => {
                if (this.groupList.length == 0) {
                    return;
                }
                let obj = this.groupList.find((list) => {
                    return list.groupId == item;
                });
        ...
        validateCategory() {
            if (this.categoryList2.length > 0 && !this.category2) {
                return false;
            } else return !(this.categoryList3.length > 0 && !this.category3);
        },
        next: function() {
